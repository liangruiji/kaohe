...
~~~json
{
  "default_locale": "en",		// 默认语言,对于含有 _locales 目录的扩展程序来说这一属性是必需的，在没有 _locales 目录的扩展程序中该属性不能存在
  "description": "纯文本描述", // 插件描述
  "icons": { // 插件的图标文件}
...
~~~js
 {  "manifest_version": 2,   //manifest版本
    ...
    "background": { // 注册后台脚本
    	...
    	"persistent": false  // false 为事件页面，默认为true为后台页面，注意，如果没有 "persistent" 键，您将得到一个普通的后台网页。是否持久存在是事件页面与后台网页之间的根本区别。
  	 },
...
https://crxdoc-zh.appspot.com/extensions/event_pages 事件页面
