        ...
        for (var t in prsr) {
            ...
            if (tab) {
                var off = tab[0]
                  , tobj = tmap[off];
                if (tobj == null)
                    tobj = prsr[t].parseTab(data, off, tab[1], obj);
                obj[t] = tmap[off] = tobj;
            }
        }
        return obj;
...
Typr["T"].cmap = {
    parseTab: function(data, offset, length) {
        ...
        for (var i = 0; i < numTables; i++) {
            ...
            if (tind == -1) {
                ...
                if (format == 0)
                    subt = cmap.parse0(data, noffset, subt);
                    //else if(format== 2) subt.off = noffset;
                else if (format == 4)
                    subt = cmap.parse4(data, noffset, subt);
                else if (format == 6)
                    subt = cmap.parse6(data, noffset, subt);
                else if (format == 12)
                    subt = cmap.parse12(data, noffset, subt);
    ...
    parse12: function(data, offset, obj) {
        ...
        //new Uint32Array(data.slice(offset, offset+nGroups*12).buffer);  
...
Typr["T"].glyf = {
    ...
    _parseGlyf: function(font, g) {
        ...
        if (gl.noc > 0) {
            ...
            } while (flags & MORE_COMPONENTS)if (flags & WE_HAVE_INSTRUCTIONS) {
...
Typr["T"].name = {
    parseTab: function(data, offset, length) {
		...
		*/
        ...
        for (var p in obj)
            if (obj[p][psn] != null && obj[p]["_lang"] == 0x0409)
                return obj[p];
        // United States
        for (var p in obj)
            if (obj[p][psn] != null && obj[p]["_lang"] == 0x0000)
                return obj[p];
        // Universal
        for (var p in obj)
            if (obj[p][psn] != null && obj[p]["_lang"] == 0x0c0c)
                return obj[p];
        // Canada
        for (var p in obj)
            if (obj[p][psn] != null)
