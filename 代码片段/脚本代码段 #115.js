...
let token = s("_m_h5_tk").split("_")[0]
// _m_h5_c

let bb = '{"dataType":"moduleData","argString":"{\\"memberId\\":\\"b2b-32469117117b9d4\\",\\"appName\\":\\"pcmodules\\",\\"resourceName\\":\\"wpOfferColumn\\",\\"type\\":\\"view\\",\\"version\\":\\"1.0.0\\",\\"appdata\\":{\\"sortType\\":\\"wangpu_score\\",\\"sellerRecommendFilter\\":false,\\"mixFilter\\":false,\\"tradenumFilter\\":false,\\"quantityBegin\\":null,\\"pageNum\\":5,\\"count\\":30}}"}'
function aa(e) {
    function t(e, t) {
        return e << t | e >>> 32 - t
    }
...
const getData = ()=>{
    ...
    let sign = aa(token + "&" + t + "&" + 12574478 + "&" + bb)
    // let kk = md5(token + "&" + t + "&" + 12574478 + "&" + bb)
    console.log(sign)
    // console.log(kk)
    if (sign) {
        let getDeleteUrl = 'https://h5api.m.1688.com/h5/mtop.1688.shop.data.get/1.0/'
        getDeleteUrl += '?jsv=2.6.2'
        getDeleteUrl += '&appKey=12574478'
        getDeleteUrl += `&t=${t}`
        getDeleteUrl += `&sign=${sign}`
        getDeleteUrl += '&api=mtop.1688.shop.data.get'
        getDeleteUrl += '&v=1.0'
        getDeleteUrl += '&type=json&valueType=string&dataType=json&timeout=10000'
        // getDeleteUrl += '&type=originaljson'
        // getDeleteUrl += '&dataType=json'
        $.ajax({
            url: getDeleteUrl,
            type: 'POST',
            dataType: 'json',
