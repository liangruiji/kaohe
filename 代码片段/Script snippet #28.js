Typr["U"] = {
    ...
    "codeToGlyph": function(font, code) {
        ...
        if (fmt == 0) {
            ...
            if (sind == -1)
                gid = 0;
            else if (code < tab.startCount[sind])
                gid = 0;
            else {
                var gli = 0;
        ...
        && [0x9, 0xa, 0xb, 0xc, 0xd, 0x20, 0x85, 0xa0, 0x1680, 0x2028, 0x2029, 0x202f, 0x3000, 0x180e, 0x200b, 0x200c, 0x200d, 0x2060, 0xfeff].indexOf(code) == -1 && !(0x2000 <= code && code <= 0x200a))
            gid = 0;
        //*/
                ...
                if (i == i0) {
                    if (onCurve) {
                        ...
                        if (prOnCurve)
                            ...
                            P.MoveTo(p, Math.floor((gl.xs[pr] + x) * 0.5), Math.floor((gl.ys[pr] + y) * 0.5));
                    }
                }
                if (onCurve) {
                    if (prOnCurve)
                    ...
                    if (nxOnCurve)
                        ...
                        P.qCurveTo(p, x, y, Math.floor((x + gl.xs[nx]) * 0.5), Math.floor((y + gl.ys[nx]) * 0.5));
                }
            }
            P.ClosePath(p);
        }
    ...
    "_drawCFF": function(cmds, state, font, pdct, p) {
        ...
        while (i < cmds.length) {
            ...
            if (false) {} else if (v == "o1" || v == "o18") //  hstem || hstemhm
            {
                var hasWidthArg;
    ...
    "SVG": function() {
        ...
        function _readTrnsAttr(fnc, vls) {
            ...
            if (false) {} else if (fnc == "translate") {
                if (vls.length == 1)
                    M.translate(m, vls[0], 0);
                else
                    M.translate(m, vls[0], vls[1]);
            } else if (fnc == "scale") {
                if (vls.length == 1)
                    M.scale(m, vls[0], vls[0]);
                else
                    M.scale(m, vls[0], vls[1]);
            } else if (fnc == "rotate") {
                var tx = 0
                  , ty = 0;
                if (vls.length != 1) {
        ...
        function _tokens(d) {
            var ts = []
              ...
              , cn = ""
              , pc = "";
            // reading number, current number, prev char
            while (off < d.length) {
